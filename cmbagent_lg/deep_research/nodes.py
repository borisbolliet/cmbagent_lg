...
def _build_step_summary(step: Step, sub_state: dict) -> str:
    ...
    a header (`### Step N (agent): sub_task`) followed by the executed code
    and its stdout. The engineer at step N+1 sees this in its prompt and can
    reference it to import/use what came before.
    """
...
def run_step(
    ...
    if step.sub_task_agent != "engineer":
        outcome = {
            ...
            "fulfilled": False,
            "reason": (
                f"non-engineer step '{step.sub_task_agent}' not supported in v1 "
                f"(only `engineer` steps are implemented)"
            ),
        }
        return {
    ...
    sub_state = self_debug_graph.invoke(
        {
            "step": step,
